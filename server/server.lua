...
AddEventHandler('rex-huntingwagon:server:sellhuntingcart', function(plate)
    ...
    if wagon[1] then
        ...
        MySQL.update('DELETE FROM inventories WHERE identifier = ?', { wagon[1].plate })
...
CreateThread(function() 
    exports['rsg-inventory']:CreateShop({
        name = 'hunting',
        label = 'Hunting Shop',
        slots = #Config.HuntingShopItems,
        items = Config.HuntingShopItems,
...
-- open shop
--------------------------------------
RegisterNetEvent('rex-huntingwagon:server:openShop', function() 
    local src = source
    local Player = RSGCore.Functions.GetPlayer(src)
    if not Player then return end
    exports['rsg-inventory']:OpenShop(src, 'hunting')
end)
